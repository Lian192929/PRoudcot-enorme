$(document).ready(() => {
    ...
    function updateEmbed(embed) {
        ...
        // add basic embed 
        source = `EmbedBuilder embedBuilder = new EmbedBuilder();`;

        if (embed.url) {
            $('.embed-inner').append(`<div class="embed-title"><a href="${embed.url}">${embed.title}</a></div>`);

            // update source
            source += `embedBuilder.setTitle("[${embed.title}](${embed.url})");`;
        } else {
            $('.embed-inner').append(`<div class="embed-title">${embed.title}</div>`);

            // update source
            source += `embedBuilder.setTitle("${embed.title}");`;
        }
        ...
        if (embed.description) {
            ...
            // update source
            source += `embedBuilder.setDescription("${embed.description}");`;
        }
        ...
        if (embed.color) {
            ...
            // update source
            source += `embedBuilder.setColor(${embed.color.substr(1)});`;
        }
        ...
        if (embed.thumb_url) {
            // add thumbnail
            source += `embedBuilder.setThumbnail(`;
        ...
        `);
            ...
            // add field
            source += `embedBuilder.addField("${field.name}", "${field.value}", ${field.inline && 'True' || 'False'});\n`;
        }
        ...
        if (embed.footer) {
            ...
            // add footer
            source += `embedBuilder.setFooter("${embed.footer}");\n`;
        }

        // add send function
        source += `MessageEmbed embed = embedBuilder.build();\n`;

        source += `System.out.println(embed);\n`;
        // code
        $('.source').text(source);
    ...
    function generateInputFields(fields) {
        ...
        for (let i = 0; i < fields; i++) {
          ...
          <div class="col-sm-4">
            <input class="form-control" id="field-${i}-name" type="text" placeholder="nombre" value="${embed.fields[i].name !== undefined ? embed.fields[i].name : ''}" />
          </div>
          <div class="col-sm-4">
            <input class="form-control" id="field-${i}-value" type="text" placeholder="valor" value="${embed.fields[i].value !== undefined ? embed.fields[i].value : ''}" />
          </div>
          <div class="col-sm-2">
            <div class="form-check">
              <label class="form-check-label">
                <input class="form-check-input" id="field-${i}-inline" type="checkbox" ${embed.fields[i].inline !== undefined ? 'checked="checked"' : ''}> En lÃ­nea
              </label>
            </div>
          </div>
          <div class="col-sm-2">
